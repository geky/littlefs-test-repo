...
// this is just a hint that size may be > lfs3_size_t
static inline lfs3_off_t lfs3_data_weight(const lfs3_data_t *data) {
    return data->weight;
}
    ...
    if (lfs3_data_ondisk(data)) {
        ...
        if (LFS3_IFDEF_CKDATACKSUMS(
            ...
            int err = lfs3_bd_readck(lfs3,
                    lfs3_data_block(data), lfs3_data_off(data),
    ...
    if (lfs3_data_ondisk(data)) {
        ...
        if (LFS3_IFDEF_CKDATACKSUMS(
            ...
            int cmp = lfs3_bd_cmpck(lfs3,
                    ...
                    // following data
                    lfs3_data_block(data), lfs3_data_off(data), 0,
                    buffer, d,
                    lfs3_data_cksize(data), lfs3_data_cksum(data));
            if (cmp != LFS3_CMP_EQ) {
                return cmp;
...
// from helpers
#ifndef LFS3_RDONLY
static inline lfs3_from_t lfs3_from_isptr(lfs3_from_t from) {
    return from & LFS3_FROM_PTR;
}
#endif
...
static inline lfs3_off_t lfs3_bptr_weight(const lfs3_bptr_t *bptr) {
    return lfs3_data_weight(&bptr->d);
}
...
static int lfs3_rbyd_appendrattr_(lfs3_t *lfs3, lfs3_rbyd_t *rbyd,
        ...
        lfs3_from_t from, const void *args) {
    // tag must not be internal at this point
    LFS3_ASSERT(lfs3_tag_suptype(tag) != LFS3_TAG_INTERNAL);
    // bit 7 is reserved for future subtype extensions
    LFS3_ASSERT(!(tag & 0x80));
    ...
    // pointer? dereference args
    const lfs3_rattr_t *args_ = args;
    if (lfs3_from_isptr(from)) {
        args_ = (const lfs3_rattr_t*)args_[0];
    }

    // little buffer?
    if (lfs3_from_from8(from) == LFS3_FROM_LBUF) {
        ctx.u.data = LFS3_DATA_BUF(
                (const uint8_t*)args_[0],
        ...
        if (lfs3_from_from2(from) == LFS3_FROM_BUF) {
            ctx.u.data = LFS3_DATA_BUF((const uint8_t*)args_[0], args_[1]);
            datas = &ctx.u.data;
            data_count = 1;
...
static int lfs3_rbyd_appendcompactrbyd(lfs3_t *lfs3, lfs3_rbyd_t *rbyd_,
    ...
    while (true) {
        ...
        int err = lfs3_rbyd_appendcompactrattr(lfs3, rbyd_,
                (const lfs3_rattr_t[]){
                    LFS3_RATTR(tag, -2, 1,
                        LFS3_FROM_DATA | LFS3_FROM_PTR, 1),
...
static void lfs3_fs_claimbtree(lfs3_t *lfs3, lfs3_btree_t *btree) {
    ...
    for (lfs3_handle_t *h = lfs3->handles; h; h = h->next) {
        if (lfs3_o_type(h->flags) == LFS3_TYPE_REG
                ...
                && ((lfs3_bshrub_t*)h)->b.blocks[0] == btree->blocks[0]) {
    ...
    split_recurse:;
        ...
        *r++ = LFS3_RATTR_ARG(sibling.cksum);
        if (lfs3_tag_suptype(split_tag) == LFS3_TAG_NAME) {
            *r++ = LFS3_RATTR(LFS3_TAG_BNAME, 0, 3, LFS3_FROM_DATA, 1);
...
#ifndef LFS3_RDONLY
static bool lfs3_shrub_islast(lfs3_t *lfs3,
        const lfs3_shrub_t *shrub) {
    const lfs3_shrub_t *last = NULL;
    for (lfs3_handle_t *h = lfs3->handles; h; h = h->next) {
        if (lfs3_o_type(h->flags) == LFS3_TYPE_REG
                && lfs3_shrub_cmp(&((lfs3_bshrub_t*)h)->b, shrub) == 0) {
            last = &((lfs3_bshrub_t*)h)->b;
        }
    }
...
static inline bool lfs3_bshrub_isbshrub(const lfs3_bshrub_t *bshrub) {
    return lfs3_shrub_isshrub(&bshrub->b);
}
...
static int lfs3_bshrub_commitroot_(lfs3_t *lfs3, lfs3_bshrub_t *bshrub,
        const lfs3_bcommit_t *bcommit) {
    // this is where shestimate becomes important, we need to prevent
    // our shrub from overflowing its mdir somehow

    // check if this shrub + any not-in-sync shrub references will
    // exceed our shrub_size

    // we should be marked as needssync here
    LFS3_ASSERT(lfs3_o_needssync(bshrub->h.flags));
    ...
    for (lfs3_handle_t *h = lfs3->handles; h; h = h->next) {
        if (lfs3_o_type(h->flags) == LFS3_TYPE_REG
                ...
                // lfs3_rbyd_estimate? note lfs3_shrub_islast doesn't
                // work as-is due to mixed sync/needssync file handles
    ...
    LFS3_ASSERT(bcommit->shestimate
            >= -(lfs3_ssize_t)((lfs3_bshrub_isbshrub(bshrub))
                ? bshrub->b.eoff
                : 0));
    // uh oh, too big?
    ...
    // in-flight!
    int err = lfs3_mdir_commit(lfs3, &bshrub->h.mdir, (const lfs3_rattr_t[]){
    ...
    LFS3_ASSERT(bshrub->b.blocks[0] == bshrub->h.mdir.r.blocks[0]);
    LFS3_ASSERT((lfs3_ssize_t)bshrub->b.eoff >= 0);
...
static int lfs3_bshrub_commit_(lfs3_t *lfs3, lfs3_bshrub_t *bshrub,
    ...
    LFS3_ASSERT(lfs3_shrub_trunk(&bshrub->b));
    #ifdef LFS3_DBGBTREECOMMITS
...
static int lfs3_bshrub_commit(lfs3_t *lfs3, lfs3_bshrub_t *bshrub,
    ...
    return lfs3_bshrub_commit_(lfs3, bshrub,
...
#ifndef LFS3_RDONLY
static inline bool lfs3_t_isstepcompactmeta(uint32_t flags) {
    return flags & LFS3_t_STEPCOMPACTMETA;
}
#endif
    ...
    while (a_rid != b_rid) {
        ...
        for (lfs3_handle_t *h = lfs3->handles; h; h = h->next) {
            if (lfs3_o_type(h->flags) == LFS3_TYPE_REG
                    ...
                    // reference
                    && lfs3_shrub_islast(lfs3, &((lfs3_bshrub_t*)h)->b)) {
                lfs3_ssize_t dsize__ = lfs3_rbyd_estimate(lfs3,
                        &((lfs3_bshrub_t*)h)->b, -1, -1,
                        NULL);
                if (dsize__ < 0) {
                    return dsize__;
                }
...
static int lfs3_mdir_commit(lfs3_t *lfs3, lfs3_mdir_t *mdir,
    ...
    for (lfs3_handle_t *h = lfs3->handles; h; h = h->next) {
        if (lfs3_o_type(h->flags) == LFS3_TYPE_REG) {
            ...
            LFS3_ASSERT(!lfs3_bshrub_isbshrub((lfs3_bshrub_t*)h)
                    || ((lfs3_bshrub_t*)h)->b.blocks[0]
                        == h->mdir.r.blocks[0]);
            ((lfs3_bshrub_t*)h)->b_ = ((lfs3_bshrub_t*)h)->b;
        }
    }
        ...
        if (lfs3->mtree.weight == 0) {
            ...
            err = lfs3_mtree_commit(lfs3, &mtree_,
                    0, (const lfs3_rattr_t[]){
                        LFS3_RATTR(LFS3_TAG_MDIR, -2, 1,
                            LFS3_FROM_MPTR | LFS3_FROM_PTR),
                        LFS3_RATTR_WEIGHT(+(1 << lfs3->mbits)),
                        LFS3_RATTR_ARG(mdir_[0].r.blocks),
                        LFS3_RATTR(LFS3_TAG_MNAME, -2, 1,
                            LFS3_FROM_DATA | LFS3_FROM_PTR, 1),
                        LFS3_RATTR_WEIGHT(+(1 << lfs3->mbits)),
                        LFS3_RATTR_ARG(&split_name),
                        LFS3_RATTR(LFS3_TAG_MDIR, 0, 1,
                            LFS3_FROM_MPTR | LFS3_FROM_PTR),
    ...
    for (lfs3_handle_t *h = lfs3->handles; h; h = h->next) {
        ...
        if (lfs3_o_type(h->flags) == LFS3_TYPE_REG) {
            ...
            ((lfs3_bshrub_t*)h)->b = ((lfs3_bshrub_t*)h)->b_;
        }
    }
    ...
    if (!lfs3_m_isrdonly(lfs3->flags)) {
        for (lfs3_handle_t *h = mtrv->h.next; h; h = h->next) {
            ...
            if (h->mdir.mid == mtrv->h.mdir.mid
                ...
                if (lfs3_o_needsgraft(file->b.h.flags)) {
        ...
        // pending traversal work?
            ...
            mgc->t.h.flags &= t | ~(
                    LFS3_IFDEF_RDONLY(0, LFS3_t_STEPMKCONSISTENT)
                        ...
                        | LFS3_IFDEF_RDONLY(0, LFS3_t_STEPCOMPACTMETA)
                        | LFS3_t_STEPCKMETA
                        | LFS3_t_STEPCKDATA);
...
static int lfs3_gbmap_set__(lfs3_t *lfs3, lfs3_btree_t *gbmap,
    ...
    int err = lfs3_gbmap_commit(lfs3, &gbmap_, bid__, (const lfs3_rattr_t[]){
            ...
            (lfs3_ecksum_isecksum(ecksum))
                ? LFS3_RATTR(tag, -2, 1,
                    LFS3_FROM_ECKSUM | LFS3_FROM_PTR)
                : LFS3_RATTR(tag, -2, 1),
...
static inline void lfs3_file_discardleaf(lfs3_file_t *file) {
    file->b.h.flags &= ~LFS3_o_NEEDSCRYST & ~LFS3_o_NEEDSGRAFT;
    file->leaf.pos = 0;
...
static int lfs3_file_fetch(lfs3_t *lfs3, lfs3_file_t *file, uint32_t flags) {
    ...
    if (!LFS3_IFDEF_RDONLY(
        ...
        lfs3_btree_t btree;
        int err = lfs3_mdir_fetchbshrub(lfs3, &file->b.h.mdir,
                &btree);
        if (err && err != LFS3_ERR_NOENT) {
            return err;
        }
        if (err != LFS3_ERR_NOENT) {
            // if we're opening for writing, we need to find an upper
            // bound on any bshrub shrubs that can be easily updated
            // during bshrub commits, we use this in lfs3_bshrub_commit
            // to determine when shrubs exceed shrub_size and need to be
            // evicted
            //
            // we don't store this on-disk as it depends on a lot of
            // internal variables, such as worst-case leb128 encoding,
            // and would be easily broken by driver changes
            #ifndef LFS3_RDONLY
        ...
        // mark as in-sync
        file->b.h.flags &= ~LFS3_o_NEEDSSYNC;
    }
...
static int lfs3_file_opencfg_(lfs3_t *lfs3, lfs3_file_t *file,
    ...
    if (LFS3_IFDEF_RDONLY(
        ...
        if (tag != LFS3_ERR_NOENT) {
            file->b.h.flags |= LFS3_o_NEEDSCREAT | LFS3_o_NEEDSSYNC;
        }
        #endif
...
static void lfs3_file_close_(lfs3_t *lfs3, lfs3_file_t *file) {
    ...
    #ifndef LFS3_RDONLY
    if (lfs3_o_needscreat(file->b.h.flags)
...
static int lfs3_file_read_(lfs3_t *lfs3, lfs3_file_t *file,
    ...
    if (pos < file->leaf.pos + lfs3_bptr_weight(&file->leaf.bptr)) {
    ...
    if (pos < file->b.b.weight) {
        ...
        if (!lfs3_o_needscryst(file->b.h.flags)
            ...
            file->leaf.pos = bid__-(lfs3_bptr_weight(&bptr__)-1);
        ...
        *data_ = lfs3_data_fromslice(&bptr__.d,
                pos - (bid__-(lfs3_bptr_weight(&bptr__)-1)),
                d);
        return 0;
    }
...
lfs3_ssize_t lfs3_file_read(lfs3_t *lfs3, lfs3_file_t *file,
    ...
    while (size_ > 0 && pos_ < lfs3_file_size_(file)) {
        ...
        if (!lfs3_o_needsflush(file->b.h.flags)
            ...
            lfs3_ssize_t d = lfs3_min(lfs3_data_weight(&data__), size_);
...
static int lfs3_file_graft__(lfs3_t *lfs3, lfs3_file_t *file,
    ...
    while (pos_ > file->b.b.weight
        ...
        if (pos_ > file->b.b.weight) {
            lfs3_off_t l_hole = pos_ - file->b.b.weight;
        ...
        // left sibling?
        if ((lfs3_soff_t)lfs3_bptr_weight(&l_bptr) > 0) {
            // left hole?
        ...
        if (dgrow) {
            ...
            // graft fragment?
                ...
                lfs3_size_t count
                        = ((lfs3_soff_t)lfs3_bptr_weight(&l_bptr) < 0)
                        + 1
                        + ((lfs3_soff_t)lfs3_bptr_weight(&r_bptr) < 0);
                *r++ = LFS3_RATTR(LFS3_TAG_DATA, -2, 3*count,
                        LFS3_FROM_DATA, count);
                *r++ = LFS3_RATTR_WEIGHT(+dgrow);
                ...
                *d++ = lfs3_data_fromslice(&bptr_.d,
                        -1,
                        dgrow - lfs3_smax(
                            -(lfs3_soff_t)lfs3_bptr_weight(&l_bptr),
                            0));
                if ((lfs3_soff_t)lfs3_bptr_weight(&r_bptr) < 0) {
                    *d = r_bptr.d;
                    d->weight = -d->weight;
                    d++;
                }
...
static int lfs3_file_graft_(lfs3_t *lfs3, lfs3_file_t *file) {
    ...
    LFS3_ASSERT(lfs3_o_needssync(file->b.h.flags));
    ...
    while (true) {
        ...
        while (pos_ < crystal_limit) {
            ...
            if ((lfs3_data_ishole(&data__)
                        ...
                        // block alignment
                        || pos_+lfs3_data_weight(&data__) - block_pos
                            > lfs3->cfg->block_size)
            ...
            if (!lfs3_data_ishole(&data__)) {
                ...
                pos_ += lfs3_data_weight(&data__);
                continue;
            }

            // found a hole? fill with zeros
            err = lfs3_bd_set(lfs3, block_, pos_ - block_pos,
                    0, lfs3_data_weight(&data__),
                    &lfs3->pcksum);
            if (err) {
                LFS3_ASSERT(err != LFS3_ERR_RANGE);
                // bad prog? try another block
...
static int lfs3_file_sync_(lfs3_t *lfs3, lfs3_file_t *file,
    ...
    LFS3_ASSERT(!lfs3_o_needscreat(file->b.h.flags)
            || lfs3_o_needssync(file->b.h.flags));
    // small unflushed files must be unsync
    LFS3_ASSERT(!lfs3_o_needsflush(file->b.h.flags)
            || lfs3_o_needssync(file->b.h.flags));
    ...
    lfs3_rattr_t *r = rattrs;
    if (lfs3_o_needssync(file->b.h.flags)) {
        // explicit name?
        if (rname) {
        ...
        } else if (lfs3_o_needscreat(file->b.h.flags)) {
        ...
        if (lfs3_file_size_(file) > 0
                && !lfs3_o_needsflush(file->b.h.flags)) {
        ...
        LFS3_ASSERT(lfs3_file_size_(file) > 0 || !file->b.b.trunk);
    ...
    bool attrs = lfs3_o_needssync(file->b.h.flags);
    if (!attrs) {
        for (lfs3_size_t i = 0; i < file->cfg->attr_count; i++) {
            // skip readonly attrs and lazy attrs
    ...
    for (lfs3_handle_t *h = lfs3->handles; h; h = h->next) {
        if (lfs3_o_type(h->flags) == LFS3_TYPE_REG
            ...
            file_->b.h.flags &= ~LFS3_o_NEEDSCREAT;

            // mark desynced files an unsynced
            if (lfs3_o_isdesync(file_->b.h.flags)) {
                file_->b.h.flags |= LFS3_o_NEEDSSYNC;
    ...
    // mark as synced
    file->b.h.flags &= ~LFS3_o_NEEDSCREAT
            & ~LFS3_o_NEEDSSYNC
            & ~LFS3_o_NEEDSFLUSH
            & ~LFS3_o_NEEDSCRYST
            & ~LFS3_o_NEEDSGRAFT;
...
static int lfs3_formatgbmap(lfs3_t *lfs3) {
    ...
    int err = lfs3_bd_erase(lfs3, lfs3->gbmap.b.blocks[0]);
    if (err) {
        return err;
    }
