...
static int lfs3_rbyd_fetch_(lfs3_t *lfs3, lfs3_rbyd_t *rbyd,
    ...
    // are we quickfetching?
    bool quickfetch = (trunk+1) & LFS3_RBYD_QUICKFETCH;
    trunk = ((trunk+1) & ~LFS3_RBYD_QUICKFETCH) - 1;
...
static int lfs3_rbyd_fetch(lfs3_t *lfs3, lfs3_rbyd_t *rbyd,
    ...
    return lfs3_rbyd_fetch_(lfs3, rbyd, block, trunk,
            NULL);
}
...
static int lfs3_rbyd_quickfetch(lfs3_t *lfs3, lfs3_rbyd_t *rbyd,
    ...
    int err = lfs3_rbyd_fetch_(lfs3, rbyd,
...
static int lfs3_bshrub_commitroot_(lfs3_t *lfs3, lfs3_bshrub_t *bshrub,
    ...
    for (lfs3_handle_t *h = lfs3->handles; h; h = h->next) {
        if (lfs3_o_type(h->flags) == LFS3_TYPE_REG
                ...
                && lfs3_bshrub_isbshrub(&((lfs3_file_t*)h)->bshrub)
            ...
            // anything else
            shestimate = lfs3_sadd(shestimate, ((lfs3_file_t*)h)->bshrub.eoff);
        }
    }
...
#ifndef LFS3_RDONLY
static inline bool lfs3_gc_isstepmkconsistent(uint32_t flags) {
    return flags & LFS3_gc_STEPMKCONSISTENT;
}
#endif

#ifndef LFS3_RDONLY
static inline bool lfs3_gc_issteplookahead(uint32_t flags) {
    return flags & LFS3_gc_STEPLOOKAHEAD;
}
#endif

#if !defined(LFS3_RDONLY) && defined(LFS3_PREERASE)
static inline bool lfs3_gc_issteppreerase(uint32_t flags) {
    return flags & LFS3_gc_STEPPREERASE;
}
#endif

#ifndef LFS3_RDONLY
static inline bool lfs3_gc_isstepcompactmeta(uint32_t flags) {
    return flags & LFS3_gc_STEPCOMPACTMETA;
}
#endif

static inline bool lfs3_gc_isstepckmeta(uint32_t flags) {
    return flags & LFS3_gc_STEPCKMETA;
}

static inline bool lfs3_gc_isstepckdata(uint32_t flags) {
    return flags & LFS3_gc_STEPCKDATA;
}
...
static int lfs3_mdir_fetch(lfs3_t *lfs3, lfs3_mdir_t *mdir,
    ...
    for (int i = 0; i < 2; i++) {
        int err = lfs3_rbyd_fetch_(lfs3, &mdir->r, blocks[0], -1,
                &mdir->gcksumdelta);
        if (err && err != LFS3_ERR_CORRUPT) {
            return err;
        }
...
static int lfs3_mtree_gc(lfs3_t *lfs3, lfs3_mgc_t *mgc) {
    ...
    if (mgc->t.h.mdir.mid == LFS3_MID_MROOTANCHOR) {
        ...
        if (lfs3_gc_issteplookahead(mgc->t.h.flags)
    ...
    if (lfs3_gc_isstepmkconsistent(mgc->t.h.flags)
    ...
    // compacting mdirs?
    if (lfs3_gc_isstepcompactmeta(mgc->t.h.flags)
            && tag == LFS3_TAG_MDIR
...
static lfs3_sblock_t lfs3_mgc_gc(lfs3_t *lfs3, lfs3_mgc_t *mgc,
    ...
    for (; steps < 0 || i < lfs3_max(steps, 1); i = lfs3_ssadd(i, 1)) {
        ...
        // pending traversal work?
            ...
            // to allocate
            #ifndef LFS3_RDONLY
            if (lfs3_gc_issteplookahead(t)) {
                t &= ~LFS3_gc_STEPMKCONSISTENT
                        & ~LFS3_gc_STEPCOMPACTMETA;
            }
            #endif
            ...
            // other operations
            mgc->t.h.flags &= t | ~(
                    LFS3_IFDEF_RDONLY(0, LFS3_gc_STEPMKCONSISTENT)
                        | LFS3_IFDEF_RDONLY(0, LFS3_gc_STEPLOOKAHEAD)
                        | LFS3_IFDEF_RDONLY(0, LFS3_gc_STEPCOMPACTMETA)
                        | LFS3_gc_STEPCKMETA
                        | LFS3_gc_STEPCKDATA);
...
static int lfs3_mtree_fixorphans(lfs3_t *lfs3) {
    ...
    // LFS3_gc_STEPMKCONSISTENT really just removes orphans
    //
    ...
    lfs3_mgc_t mgc;
    lfs3_mgc_init(&mgc,
            LFS3_GC_WRONLY | LFS3_T_MTREEONLY | LFS3_gc_STEPMKCONSISTENT);
    while (true) {
        int err = lfs3_mtree_gc(lfs3, &mgc);
        if (err) {
            if (err == LFS3_ERR_NOENT) {
...
static lfs3_sblock_t lfs3_alloc__(lfs3_t *lfs3, uint32_t flags,
    ...
    while (true) {
        ...
        //
        lfs3_mtrv_t mtrv;
        lfs3_mtrv_init(&mtrv, LFS3_T_RDONLY | LFS3_gc_STEPLOOKAHEAD);
        while (true) {
            lfs3_bptr_t bptr;
