...
static inline bool lfs3_data_isbuf(const lfs3_data_t *data) {
    return !(data->size & (LFS3_DATA_ONDISK | LFS3_DATA_ISHOLE));
}
...
static inline bool lfs3_bptr_ondisk(const lfs3_bptr_t *bptr) {
    return bptr->d.size & LFS3_BPTR_ONDISK;
}
...
static inline bool lfs3_bptr_isbptr(const lfs3_bptr_t *bptr) {
    return (bptr->d.size & (LFS3_BPTR_ONDISK | LFS3_BPTR_ISBPTR))
            == (LFS3_BPTR_ONDISK | LFS3_BPTR_ISBPTR);
}
    ...
    while (pos_ > file->b.b.weight || cut_ > 0 || grow_ > 0) {
        ...
        while (poke < lfs3_min(pos_+cut_+1, file->b.b.weight)) {
            ...
            if (snip) {
                ...
                dcut += lfs3_min(
                        lfs3_min(lfs3_bptr_size(&bptr__), bid__+1 - pos_),
                        cut_ - dcut);
...
static int lfs3_file_graft_(lfs3_t *lfs3, lfs3_file_t *file) {
    ...
    int err = lfs3_file_graft__(lfs3, file,
            file->leaf.pos, lfs3_bptr_size(&file->leaf.bptr),
            &file->leaf.bptr, lfs3_bptr_size(&file->leaf.bptr));
    if (err) {
        return err;
    }
